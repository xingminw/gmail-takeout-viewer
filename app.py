...
function bindFilters(){document.querySelectorAll('.filter').forEach(b=>b.onclick=()=>{state.filterType=b.dataset.type; state.filterValue=b.dataset.value; state.page=1; document.querySelectorAll('.filter').forEach(x=>x.classList.toggle('active',x===b)); loadList();});}
